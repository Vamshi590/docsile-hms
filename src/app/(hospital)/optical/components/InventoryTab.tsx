...
const CATEGORIES = ["Frame", "Lens", "Contact Lens", "Accessory", "Solution", "Sunglasses"]
const FRAME_TYPES = ["Full Rim", "Half Rim", "Rimless", "Wrap"]
const FRAME_MATERIALS = ["Metal", "Plastic", "TR-90", "Titanium", "Acetate", "Memory Metal"]
const LENS_TYPES = ["Single Vision", "Bifocal", "Progressive", "Photochromic", "Tinted"]
const LENS_MATERIALS = ["CR-39", "Polycarbonate", "Glass", "Trivex", "Hi-Index"]
const LENS_COATINGS = ["Anti-Reflection", "Blue Cut", "Photochromic", "Hardcoat", "Mirror"]
const LENS_INDICES = ["1.5", "1.56", "1.6", "1.67", "1.74"]
const CL_TYPES = ["Daily", "Monthly", "Yearly", "Toric", "Multifocal"]

type Product = {
  id: string; name: string; brand: string | null; category: string; type: string | null
  material: string | null; color: string | null; size: string | null; coating: string | null
  index: string | null; modelNumber: string | null; hsnCode: string | null; gstPercent: number
}

type StockEntry = {
  id: string; productId: string; batchNumber: string | null; quantity: number
  mrp: number; costPrice: number; gstPercent: number; power: string | null
  product: Product
}
...
export function InventoryTab({ onStockChanged }: { onStockChanged?: () => void }) {
  ...
  const [productForm, setProductForm] = useState({
    name: "", brand: "", category: "Frame", type: "", material: "",
    color: "", size: "", coating: "", index: "", modelNumber: "", hsnCode: "", gstPercent: "12",
  })

  // Add stock form
  const [stockForm, setStockForm] = useState({
    productId: "", batchNumber: "", quantity: "1", mrp: "", costPrice: "", gstPercent: "12", power: "",
  })
  ...
  async function loadData() {
    ...
    const [stockData, productData] = await Promise.all([
      getOpticalStock({ search: search || undefined, category: filterCategory !== "All" ? filterCategory : undefined, lowStock: filterLowStock }),
      getOpticalProducts(),
    ])
  ...
  async function handleCreateProduct() {
    ...
    if (result.success) {
      ...
      setProductForm({ name: "", brand: "", category: "Frame", type: "", material: "", color: "", size: "", coating: "", index: "", modelNumber: "", hsnCode: "", gstPercent: "12" })
      loadData()
    } else {
  ...
  async function handleAddStock() {
    if (!stockForm.productId) { toast.error("Select a product"); return }
    if (!stockForm.mrp) { toast.error("MRP is required"); return }
    setSavingStock(true)
    ...
    if (result.success) {
      ...
      setStockForm({ productId: "", batchNumber: "", quantity: "1", mrp: "", costPrice: "", gstPercent: "12", power: "" })
      loadData()
      onStockChanged?.()
    } else {
      toast.error(result.error)
  ...
  async function handleInlineEdit(stockId: string, field: string, value: string) {
  ...
  const typeOptions = productForm.category === "Frame" ? FRAME_TYPES
    : productForm.category === "Lens" ? LENS_TYPES
    : productForm.category === "Contact Lens" ? CL_TYPES
    : []

  const materialOptions = productForm.category === "Frame" ? FRAME_MATERIALS
    : productForm.category === "Lens" ? LENS_MATERIALS
    : []
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button size="sm" className="h-9 gap-1.5" onClick={() => setShowProductDialog(true)}>
          <Plus className="h-3.5 w-3.5" /> Add Product
        </Button>
        <Button size="sm" variant="outline" className="h-9 gap-1.5" onClick={() => setShowStockDialog(true)}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-xs text-muted-foreground uppercase">
                ...
                <th className="text-left px-2 py-2.5 font-medium w-20">Category</th>
                <th className="text-left px-2 py-2.5 font-medium w-24">Batch</th>
                <th className="text-center px-2 py-2.5 font-medium w-16">Qty</th>
                <th className="text-right px-2 py-2.5 font-medium w-24">MRP</th>
                <th className="text-right px-2 py-2.5 font-medium w-24">Cost</th>
                <th className="text-right px-3 py-2.5 font-medium w-24">Value</th>
              </tr>
            </thead>
            <tbody>
              {stock.map((item) => (
                <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                  <td className="px-3 py-2.5">
                    <p className="font-medium">
                      {item.product.brand && <span className="text-muted-foreground">{item.product.brand} </span>}
                      {item.product.name}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {[item.product.type, item.product.material, item.product.color, item.product.size, item.product.modelNumber, item.power]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                  </td>
                    ...
                    <Input
                      ...
                      onBlur={(e) => {
                        if (e.target.value !== String(item.costPrice)) {
                          handleInlineEdit(item.id, "costPrice", e.target.value)
                        }
                      }}
                      className="h-7 w-24 text-right ml-auto px-2 border-0 shadow-none focus-visible:ring-1 focus-visible:ring-gray-200 bg-transparent tabular-nums"
                    />
      ...
      <Dialog open={showProductDialog} onOpenChange={setShowProductDialog}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1.5">
                ...
                <Select value={productForm.category} onValueChange={(v) => setProductForm((p) => ({ ...p, category: v, type: "", material: "", coating: "", index: "" }))}>
              ...
              {materialOptions.length > 0 && (
                <div className="space-y-1.5">
                  ...
                  <Select value={productForm.material} onValueChange={(v) => setProductForm((p) => ({ ...p, material: v }))}>
                    <SelectTrigger className="focus:ring-1 focus:ring-gray-200 focus:ring-offset-0"><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>
            ...
            {productForm.category === "Lens" && (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Coating</Label>
                  <Select value={productForm.coating} onValueChange={(v) => setProductForm((p) => ({ ...p, coating: v }))}>
                    <SelectTrigger className="focus:ring-1 focus:ring-gray-200 focus:ring-offset-0"><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>
                      {LENS_COATINGS.map((c) => (
                        <SelectItem key={c} value={c}>{c}</SelectItem>
                ...
                <div className="space-y-1.5">
                  <Label>Index</Label>
                  <Select value={productForm.index} onValueChange={(v) => setProductForm((p) => ({ ...p, index: v }))}>
                    <SelectTrigger className="focus:ring-1 focus:ring-gray-200 focus:ring-offset-0"><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>
                      {LENS_INDICES.map((i) => (
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-1.5">
                <Label>{productForm.category === "Frame" ? "Size (Eye-Bridge-Temple)" : "Size"}</Label>
                <Input
                  value={productForm.size}
                  onChange={(e) => setProductForm((p) => ({ ...p, size: e.target.value }))}
                  placeholder={productForm.category === "Frame" ? "e.g. 52-18-140" : ""}
                  className="focus-visible:ring-1 focus-visible:ring-gray-200 focus-visible:ring-offset-0"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setProductForm((p) => ({ ...p, modelNumber: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setProductForm((p) => ({ ...p, hsnCode: e.target.value }))}
                  className="focus-visible:ring-1 focus-visible:ring-gray-200 focus-visible:ring-offset-0"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setProductForm((p) => ({ ...p, gstPercent: e.target.value }))}
                  className="focus-visible:ring-1 focus-visible:ring-gray-200 focus-visible:ring-offset-0"
                />
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowProductDialog(false)}>Cancel</Button>
            <Button onClick={handleCreateProduct} disabled={savingProduct}>
              {savingProduct && <Loader2 className="h-4 w-4 animate-spin mr-1.5" />}
              Add Product
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Select value={stockForm.productId} onValueChange={(v) => setStockForm((p) => ({ ...p, productId: v }))}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setStockForm((p) => ({ ...p, power: e.target.value }))}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => setStockForm((p) => ({ ...p, gstPercent: e.target.value }))}
                className="w-24 focus-visible:ring-1 focus-visible:ring-gray-200 focus-visible:ring-offset-0"
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowStockDialog(false)}>Cancel</Button>
            <Button onClick={handleAddStock} disabled={savingStock}>
              {savingStock && <Loader2 className="h-4 w-4 animate-spin mr-1.5" />}
              Add Stock
            </Button>
          </DialogFooter>
        </DialogContent>
